...
①ユーザー名とメールアドレスどちらも入れられるinput要素を設定してください。
②パスワードを入力するinput要素を実装してください（課題25と同じ。）。
③リンク要素"パスワード忘れてた方"と”forgot-password.html”を作成して遷移できるようにしてください。ページ内の実装はしなくて良いです。
④リンク要素"会員登録はこちらから"から課題24,25で作った"register.html"へ遷移できるようにしてください。
⑤mockAPIにダミーのユーザーデータを登録してください。　
　※mockAPIについては勉強会のハンズオンを先に済ませることをお勧めします。
（ユーザーデータの例）
[
    {
        "name": "Hoeger",
...
⑥fetchAPIを使ってユーザーデータを全て取得してください。
⑦ログインボタンをクリックした後、password等のデータが登録データと一致するか確認してください。
　確認の処理はサーバーで行うことを想定し、setTimeoutを使って１秒後に実行してください。
　※ログイン時に使用するデータはプルリクエストにコメントしてください。
（レビュワーに伝えるデータの例）
{
  userId: "Zella_Homenick38@example.net"
  password: "HQnmjPKBWkqzjeB"
}
⑧password等のデータが登録データと一致した場合は、tokenを発行してlocalStorageに格納して、yahoo風コンテンツページに遷移させてください。tokenの値はuserIdとしてください。
⑨password等のデータが登録データと一致しなかった場合は、事前に作成したログイン失敗ページへ遷移するようにしてください。
⑩token発効後に再度ログインページを表示した時、localStorageにトークンが格納されていればyahoo風コンテンツページへ遷移するようにしてください。
⑪yahoo風コンテンツページにログアウトボタンをつけ、このボタンを押したらローカルストレージに格納のtokenを削除するようにしてください。
※ローカルストレージに関しての危険性は"HTML5のLocal Storageを使ってはいけない（翻訳）
"(https://techracho.bpsinc.jp/hachi8833/2024_04_05/80851)で確認できます。ここではローカルストレージはあくまで学習の為に使います。
